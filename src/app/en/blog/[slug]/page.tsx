...
export default function BlogPostPage() {
  ...
  const meta = slug ? findPostMeta(slug) : null;
  const content = slug ? getPostContent(slug, "en") : null;
    ...
    return (
      <div className="mx-auto max-w-6xl px-4 py-16">
        <div className="glass rounded-2xl p-6">
          ...
          <p className="mt-2 text-platinum-300">The link may be outdated</p>
          <Link href="/en/blog" className="mt-4 inline-block badge-chip">
            ← Back to blog
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 md:py-12">
      ...
      <div className="mt-10 flex flex-wrap items-center justify-between gap-3">
        <Link href="/en/blog" className="badge-chip">
          ← Back to blog
        </Link>
        <Link href="/en/performance" className="cta-button w-auto">
          View performance <span className="arrow">→</span>
        </Link>
      </div>
    </div>
  );
