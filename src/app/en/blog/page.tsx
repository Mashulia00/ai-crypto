...
// якщо додаси *_ru в метадані — EN просто ігнорує
const titleOf   = (p: PostMeta & any) => p.title;
const excerptOf = (p: PostMeta & any) => p.excerpt;
const tagsOf    = (p: PostMeta & any) => p.tags as string[];

const fmtDate = (d: string) =>
  new Date(d).toLocaleDateString("en-GB", { day: "2-digit", month: "2-digit", year: "numeric" });

const readTime = (text: string, wpm = 220) =>
  `${Math.max(1, Math.round(text.split(/\s+/).length / wpm))} min`;
...
function PostCard({ p, index }: { p: PostMeta & any; index: number }) {
  ...
  const tags = tagsOf(p);
  return (
    <Link href={`/en/blog/${p.slug}`} className="block outline-none">
      <article
...
export default function BlogPage() {
  ...
  const allTags = useMemo(() => {
    ...
    postsIndex.forEach((p) => (p as any).tags.forEach((t: string) => set.add(t)));
    return Array.from(set).sort((a, b) => a.localeCompare(b, "en"));
  }, []);

  const filtered = useMemo(() => {
    const byTag = activeTag
      ? postsIndex.filter((p: any) => (p.tags as string[]).includes(activeTag))
      : postsIndex;
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 md:py-12">
      <SectionReveal cascade>
        <header className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-b from-white/5 to-white/10 p-5 md:p-7">
          <h1 className="text-balance text-3xl font-semibold tracking-tight md:text-5xl">
            <span className="mark-underline mark-underline--thick">Blog</span> / Education
          </h1>
          <p className="mt-3 max-w-3xl text-platinum-200/90 md:text-lg">
            Short articles about risk approach, ML, and methodology — mobile first with micro animations
          </p>
          <div
            aria-hidden
      ...
      <div className="mt-6 space-y-5">
        ...
        {filtered.length === 0 && (
          <div className="glass rounded-2xl p-6 text-platinum-300">Nothing found</div>
        )}
      </div>
    </div>
  );
