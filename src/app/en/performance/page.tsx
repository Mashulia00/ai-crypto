...
const fmtDM = (x: any) =>
  new Date(toMs(x)).toLocaleDateString("en-GB", { day: "2-digit", month: "2-digit" });
    ...
    <div className="mx-auto max-w-6xl px-4 py-10 md:py-12">
      ...
      <SectionReveal cascade>
        <header className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-b from-white/5 to-white/10 p-5 md:p-7">
          <span className="badge-chip">Demo data</span>

          <div className="relative mt-3">
            <h1 className="text-balance text-3xl font-semibold tracking-tight md:text-5xl">
              <span className="mark-underline mark-underline--thick">Performance</span> (demo)
            </h1>
          ...
          <p className="mt-4 max-w-3xl text-platinum-200/90 md:text-lg">
            PnL and drawdown charts with demo data and a short methodology
          </p>
      ...
      <div className="mt-8 grid gap-6 md:grid-cols-2">
        ...
        <SectionReveal>
          <div className="group glass rounded-2xl p-4 md:p-5 transition-shadow duration-200 hover:shadow-[0_0_30px_rgba(57,208,255,.08)]">
            <p className="mb-2 text-sm text-platinum-400">PnL (demo)</p>
            {isLoading ? (
        ...
        <SectionReveal delay={0.08}>
          <div className="group glass rounded-2xl p-4 md:p-5 transition-shadow duration-200 hover:shadow-[0_0_30px_rgba(57,208,255,.08)]">
            <p className="mb-2 text-sm text-platinum-400">Drawdown % (demo)</p>
            {isLoading ? (
              <Skeleton className="h-[280px]" />
            ) : (
              <div className="h-[280px]">
      ...
      <SectionReveal delay={0.12}>
        <div className="mt-8 cta-cyber rounded-2xl p-6">
          <h2 className="text-xl font-semibold md:text-2xl">
            <span className="mark-underline">Methodology</span> (demo)
          </h2>
          <ul className="mt-3 space-y-2 text-platinum-200">
            <li className="flex items-start gap-2">
              <span className="bullet-dot mt-2" />
              <span>Commissions and simulated slippage included</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="bullet-dot mt-2" />
              <span>Different market regimes (trend flat volatility) are modeled</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="bullet-dot mt-2" />
              <span>Data created for educational purposes only this is not investment advice</span>
            </li>
          </ul>
        </div>
      </SectionReveal>
