...
// демо-рядки для красивого вигляду
...
const fmtDM = (x: any) =>
  new Date(toMs(x)).toLocaleDateString("uk-UA", { day: "2-digit", month: "2-digit" });

// універсальний мапер з автоперевіркою «плоскої» серії
function mapPnL(arr?: any[]): PnLPoint[] {
  ...
  // якщо все ~нуль (або бекенд дав «майже нуль») — малюємо демо
...
export default function PerformancePage() {
  ...
  const pnlDomain: [number, number] = [Math.floor(pMin - pad), Math.ceil(pMax + pad)];
    ...
    <div className="mx-auto max-w-6xl px-4 py-10 md:py-12">
      ...
      <SectionReveal cascade>
        <header className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-b from-white/5 to-white/10 p-5 md:p-7">
          <span className="badge-chip">Демо-дані</span>

          <div className="relative mt-3">
            <h1 className="text-balance text-3xl font-semibold tracking-tight md:text-5xl">
              <span className="mark-underline mark-underline--thick">Продуктивність</span> (демо)
            </h1>

            {/* світлова смуга під заголовком (альтернатива) */}
            <span
              aria-hidden
                  ...
                  <AreaChart data={pnl}>
                    <defs>
                      <linearGradient id="pnlG" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor="#39d0ff" stopOpacity={0.55} />
                    ...
                    <XAxis
                      ...
                      tickFormatter={fmtDM}
                    />
                    <YAxis domain={pnlDomain} tick={{ fill: "rgba(255,255,255,.65)" }} />
                    <Tooltip
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={dd}>
                    ...
                    <XAxis
                      ...
                      tickFormatter={fmtDM}
                    />
                    <YAxis domain={ddDomain} tick={{ fill: "rgba(255,255,255,.65)" }} />
                    <Tooltip
                      labelFormatter={(v) => fmtDM(v)}
                      contentStyle={{
                        background: "rgba(0,0,0,.72)",
      ...
      <SectionReveal delay={0.12}>
        <div className="mt-8 cta-cyber rounded-2xl p-6">
          <h2 className="text-xl font-semibold md:text-2xl">
            <span className="mark-underline">Методологія</span> (демо)
          </h2>
          <ul className="mt-3 space-y-2 text-platinum-200">
            <li className="flex items-start gap-2">
              <span className="bullet-dot mt-2" />
              <span>Ураховані комісії та умовне проскальзування.</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="bullet-dot mt-2" />
              <span>Моделюються різні ринкові режими (тренд/флет/вола).</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="bullet-dot mt-2" />
              <span>Дані створено з навчальною метою; це не інвестпорада.</span>
            </li>
          </ul>
        </div>
      </SectionReveal>
